          ...
          <div className="grid gap-5 sm:grid-cols-3">
            {signatureEvents.map((ev) => (
              <Link
                key={ev.slug}
                href={`/events/${ev.slug}`}
                className="group rounded-2xl border border-slate-100 dark:border-white/10 bg-slate-50 dark:bg-white/5 p-6 hover:shadow-soft transition-all duration-300 hover:-translate-y-1 block"
              >
                <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-[#D41B69] mb-3">
                  <span>{ev.month} {ev.year}</span>
                </div>
                <h3 className="font-bold text-[#0B1426] dark:text-white text-base group-hover:text-[#D41B69] transition-colors">{ev.title}</h3>
                <p className="text-xs text-slate-500 dark:text-white/50 mt-2">{ev.tagline}</p>
              </Link>
            ))}
          </div>
        </div>
